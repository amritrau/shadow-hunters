...
def test_chocolate():
    ...
    for _ in range(C.N_ELEMENT_TESTS):
        ...
        weak_c = ["Allie", "Ellen", "Ultra Soul"]
        weak = [p for p in gc.players if p.character.name in weak_c][0]
        strong = [p for p in gc.players if p.character.name not in weak_c][0]
